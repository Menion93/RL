...
class NNLinearRegressor:
  def __init__(self, dim):
    self.W = tf.Variable(tf.random_normal((1, dim)))
    self.optimizer = tf.train.GradientDescentOptimizer(0.001)
    self.dim = dim
  
  def mse_loss(self, X, y):
    y_ = tf.matmul(self.W, X.T.astype('float32'))
    y_ = tf.reshape(y_, [-1])
    err = y - y_
  ...
  def predict(self, X):
    return tf.matmul(self.W, X.T.astype('float32'))
...
class RBFAgent:
  def __init__(self, num_actions, state_dim):
    ...
    self.action2regressor = dict([(action, NNLinearRegressor(state_dim)) 
...
if __name__ == '__main__':
  ...
  # Show learned policy
  env = wrappers.Monitor(env, './video/cartpole-tf')
